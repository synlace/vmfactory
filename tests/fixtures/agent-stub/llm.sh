#!/usr/bin/env bash
# Test stub: the agent model transcript. Call 1 = seed an image (the
# host supplies it), call 2 = a shell command turn, call 3+ = done.
d="$(cd "$(dirname "$0")" && pwd)"
n=$(cat "$d/calls" 2>/dev/null || echo 0); n=$((n+1)); echo "$n" > "$d/calls"
case "$n" in
  1) echo '{"thought":"need the app image","cmd":null,"seed_images":["ghost:5"],"done":false,"plan":null}' ;;
  2) echo '{"thought":"write the index","cmd":"mkdir -p /srv && printf agent-made > /srv/index.html && (nohup /vmf/busybox httpd -f -p 8021 -h /srv &)","done":false,"plan":null}' ;;
